...
encode_word(const std::string &word, const piece_map_t &model) {
  ...
  if (best_segmentations_scores.back() ==
      -std::numeric_limits<float>::infinity()) {
    return std::make_pair(std::vector<std::size_t>{0}, 0.0);
  }
...
bool replit_model_load(const std::string &fname, replit_model &model,
  ...
  // prepare memory for the weights
  {
    ...
    for (int i = 0; i < n_layer; ++i) {
      ...
      // map by name
      model
          .tensors["transformer.blocks." + std::to_string(i) + ".ln_1.weight"] =
          layer.ln_1_weight;
      ...
      model
          .tensors["transformer.blocks." + std::to_string(i) + ".ln_2.weight"] =
          layer.ln_2_weight;
      model.tensors["transformer.blocks." + std::to_string(i) +
...
bool replit_eval(const replit_model &model, const int n_threads,
  ...
  for (int il = 0; il < n_layer; ++il) {
    ...
    // a = self.ln_1(x)
    {
      ...
      cur = ggml_mul(ctx0, ggml_repeat(ctx0, model.layers[il].ln_1_weight, cur),
                     cur);
    }
    ...
    //  is_causal=is_causal)
    {
      ...
      // V_trans = Vmem.view(n_embd/n_head, n_head, n_past + N).permute(1, 2,
      // 0, 3).contiguous() [n_past + N, 64, 12]
    ...
    // m = self.ln_2(x)
    {
      ...
      cur = ggml_mul(ctx0, ggml_repeat(ctx0, model.layers[il].ln_2_weight, cur),
                     cur);
    }
