...
#include <iostream>
#include <unistd.h>
#include "ggml.cpp/examples/gpt-neox/main.cpp"

struct gpt_neox_state {
    gpt_vocab vocab;
...
int gpt_neox_predict(void* params_ptr, void* state_pr, char* result) {
    ...
    gpt_neox_state state = *(gpt_neox_state*) state_pr;
    gpt_vocab vocab = state.vocab;
    gpt_neox_model model = state.model;
...
// load the model
{
    ...
    const int64_t t_start_us = ggml_time_us();
    if (! gpt_neox_model_load(model_path, state->model, state->vocab)) {
...
void*  gpt_neox_allocate_state() {
    return new  gpt_neox_state;
}

void  gpt_neox_free_model(void *state_ptr) {
     gpt_neox_state* state = ( gpt_neox_state*) state_ptr;
...
void*  gpt_neox_allocate_params(const char *prompt, int seed, int threads, int tokens, int top_k,
