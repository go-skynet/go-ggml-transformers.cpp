...
int redpajama_predict(void* params_ptr, void* state_pr, char* result) {
    ...
    redpajama_state state = *(redpajama_state*) state_pr;
    gpt_vocab vocab = state.vocab;
    redpajama_model model = state.model;
...
// load the model
{
    ...
    const int64_t t_start_us = ggml_time_us();
    if (!redpajama_model_load(model_path, state->model, state->vocab)) {
...
void* redpajama_allocate_state() {
    return new redpajama_state;
}

void redpajama_free_model(void *state_ptr) {
    redpajama_state* state = (redpajama_state*) state_ptr;
...
void* redpajama_allocate_params(const char *prompt, int seed, int threads, int tokens, int top_k,
